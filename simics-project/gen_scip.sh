...
#   bash simics-project/gen_scip.sh <module_name> [output_dir]
#
# Example:
#   bash simics-project/gen_scip.sh wdt
#
# `output_dir` is a directory: dfa writes one <root-name>.scip file per
# workspace root into it (see --scip-output in src/dfa/main.rs).
set -euo pipefail

# ── args ─────────────────────────────────────────────────────────────────────

MODULE="${1:?Usage: $0 <module_name> [output_dir]}"
SCIP_OUT_ARG="${2:-}"
...
SCIP_OUT="${SCIP_OUT_ARG:-${SCRIPT_DIR}/scip_out}"
...
echo "SCIP index directory: $SCIP_OUT"
